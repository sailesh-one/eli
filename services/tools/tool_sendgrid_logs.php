...
include($_SERVER['DOCUMENT_ROOT']."/tools_india/config.php");
	...
	$connection = mysqli_connect( $db_host, $db_user, $db_pass, $db_name);
  	...
  	$query = "select count(*) as cnt from  sendgrid_logs order by id desc "  ;     
	...
	$query = "select * from sendgrid_logs order by id desc ". $limit ;	
...
<html>
<head> 
<title>BMW India SendGrid Log</title>
	...
	<div>
		...
		<table class='data-table' cellpadding="5" cellspacing="1" width="90%" align='center'>
			...
			<tr>   
				...
				<td style="width:150px;"><?=$data["mail_subject"]?></td>				
				<td><?=$data["mail_content"]?></td>				
				<td><?=$data["params"]?></td>
				<td><?=$data["response"]?></td>
				<td style="width:100px;"><?=$data["ip"]?></td>				              
			</tr>
			<?php } ?>
