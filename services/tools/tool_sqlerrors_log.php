<h1> BMW SQL Errors Log - Under Develpment</h1>
...
exit;
	...
	$query = "select * from sql_error_log order by id desc ". $limit ;	
...
$host = "indonesia";
$hosts = array(
        "Indonesia SQL Errors"=>"indonesia",
        "Thailand SQL Errors"=>"thailand",        
        "Malaysia SQL Errors" => "malaysia",
        );
				...
				<table class='data-table' cellpadding="5" cellspacing="1" style="background-color:#cdcdcd" width="90%" align='center'>
					<tr>   					  
						...
						<td style="backgound-color:white;">
							<?php if($display_page_number>1){ ?><a href='?host=<?=$host?>&p=1'>First&nbsp;|&nbsp;</a><?php } ?>
							<?php if( $display_page_number <= $pages && $display_page_number > 1 ){ ?><a href='?host=<?=$host?>&p=<?= $display_page_number-1 ?>' > Prev &nbsp;|&nbsp;</a><?php } ?>
							<?php if( $display_page_number < $pages ){ ?><a href='?host=<?=$host?>&p=<?= $display_page_number+1  ?>' > Next&nbsp;|&nbsp;</a><?php } ?>
							<?php if( $display_page_number < $pages ){ ?><a href='?host=<?=$host?>&p=<?=$pages ?>'> Last</a><?php } ?> </td>						
					</tr>
				</table> 			      	
			</form>
		</div>
