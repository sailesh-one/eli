  ...
  methods: {
    ...
    async processQueue() {
      ...
      try {
        ...
        // Upload image (pass imgPart if backend expects it)
        const result = await this.store.uploadImage(key, imgData.file, imgData.imgPart);
        ...
        if (result.success) {
          ...
          this.store.detail.images = {
            ...this.store.detail.images,
            [key]: {
              id: result.data?.id || imgData.id,
              url: result.data?.url || imgData.src,
              imgPart: imgData.imgPart
            }
          };
        } else {
    ...
    async handleDeleteConfirmed() {
      ...
      if (result.success) {
        ...
        this.deleteTarget = { key: null, label: "", src: "", id: null };
      }
    },
    ...
    <div class="container p-0 m-0 ps-2">
      ...
      <div v-if="store.isProcessing && Object.keys(store.getImagesConfig || {}).length === 0" class="row g-4 justify-content-center">
        <div v-for="n in 8" :key="n" class="col-6 col-md-4 col-lg-3">
          <div class="card border-0 shadow-sm h-100 rounded-1 overflow-hidden">
      ...
      <div class="row g-4 justify-content-center">
        <div v-for="(img, key) in mergedImagesConfig" :key="key" class="col-6 col-md-4 col-lg-3">
          <div class="card border border-light-subtle shadow-sm h-100 rounded-1 position-relative overflow-hidden">
            <!-- Delete button for existing images -->
            <button 
              v-if="!isReadOnly && img.src && !img.file" 
              class="btn btn-sm btn-light text-danger shadow-sm position-absolute top-0 end-0 m-2 z-1 px-2 py-1"
        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content rounded-4 border-0 shadow-lg">
            ...
            <div class="modal-footer border-0 pt-2">
             ...
             <button type="button" class="btn btn-sm btn-danger" @click="handleDeleteConfirmed" :disabled="store.isProcessing">
              <span v-if="store.isProcessing" class="spinner-border spinner-border-sm me-2"></span>
