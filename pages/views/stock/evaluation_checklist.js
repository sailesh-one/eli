export const EvaluationChecklist = {
    ...
    computed: {                
        templates(){                                                 
            return this.store?.getDetails?.refurb?.templates || {}
        },
        mapped_templates(){
            return this.store?.getDetails?.refurb?.mapped_templates || {}
        },
    ...
    methods: {
        async fetchChecklist()
        {
            ...
            formData.append("action", "getleadrefurbishment");                            
            formData.append("id", this.lead_id);
            const res = await $http('POST', `${g.$base_url_api}/my-stock`, formData);
    ...
    <div class="container-fluid mt-5 overflow-auto">
        ...
        <div v-else>
        <div class="mb-4">
            <label class="form-label fw-bold mb-2">Evaluation Templates</label>
            <div class="d-flex flex-wrap gap-2">
                ...
                <h3>Total Refurb Cost:₹ {{NumberFormat(overall_refurbcost)}}</h3>
            </div>
        </div>
        <div class="col-md-10" v-if="template_id in checklist">
            ...
            <div class="d-flex justify-content-end mt-3">            
                <button type="button" class="btn" style="background:#111; color:#fff; min-width:120px;" v-on:click="saveChecklist" :disabled="isProcessing" >
